...
const UIManager = {
    ...
    updateActionButtonsBasedOnPhase: function() {
        ...
        // === INICIO: BLOQUE CORREGIDO para floatingCreateDivisionBtn (¡SOLO DESPLIEGUE HUMANO!) ===
        if (floatingCreateDivisionBtn) {
            let canShowButton = false; 
            let isButtonEnabled = false; 

            // El botón de crear unidad solo debe ser visible y habilitado si:
            // 1. Estamos en la fase de DESPLIEGUE.
            // 2. El JUGADOR ACTUAL es HUMANO.
            // 3. Aún no ha alcanzado su límite de unidades desplegadas.
            if (isDeployment && gameState.playerTypes[`player${gameState.currentPlayer}`] === 'human') {
                if (gameState.unitsPlacedByPlayer && gameState.deploymentUnitLimit !== undefined) {
                    ...
                    canShowButton = true; // Límite ilimitado o no definido
                    isButtonEnabled = true;
                }
            }
            // En cualquier otra fase (Play, GameOver) o si la IA está desplegando, el botón estará oculto/deshabilitado.
